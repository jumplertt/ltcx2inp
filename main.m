...
% Prompts user to specify strut diameter
strutdia = str2double(cell2mat(inputdlg('Stut diameter (mm)', ...
	'Please specify strut diameter', [1 30],...
	{'1'})));
...
for i=1:ustruts
   for j=1:size(ssolid,1)
       snodes(j,:,i) = [j, ssolid(j,1:2)*strutdia, ssolid(j,3).*usections(i,2)];
   end
   selements (:,:,i) = esolid;
end
        ...
        for n=1:rn(end)
            if mod(n,2) == 0
                fprintf(fid3,['** Constraint: N-S-%u\n*Coupling, constraint name='...
                    ...
                    '*Kinematic\n']...
                    , n, n, n/2,(mod(n,2)*-1)+2, n, n/2, mod(n,2)+1);
            else
                fprintf(fid3,['** Constraint: N-S-%u\n*Coupling, constraint name='...
                    'N-S-%u, ref node=WS-%u%u-NsetInt-%u, surface=SS-%u-Surf-S%u\n'...
                    '*Kinematic\n']...
                    , n, n, (n/2)+0.5,(mod(n,2)*(-1))+2, n,(n/2)+0.5, mod(n,2)+1);
            end
        end
    case 'Explicit'
        for n=1:rn(end)
            if mod(n,2) == 0
                fprintf(fid3,['** Constraint: N-S-%u\n*Coupling, constraint name='...
                    'N-S-%u, ref node=WS-%u%u-NsetInt-%u, surface=SS-%u-Surf-S%u\n'...
                    '*Kinematic\n']...
                    , n, n, n/2, (mod(n,2)*-1)+2, n, n/2,...
                    mod(n,2)+2);
            else
                fprintf(fid3,['** Constraint: N-S-%u\n*Coupling, constraint name='...
                    'N-S-%u, ref node=WS-%u%u-NsetInt-%u, surface=SS-%u-Surf-S%u\n'...
                    '*Kinematic\n']...
                    , n, n, (n/2)+0.5, (mod(n,2)*(-1))+2, n,...
                    (n/2)+0.5, mod(n,2));
            end
        end        
end
